...
public class MemberQueryRepositoryImpl implements MemberQueryRepository {
    ...
    public void deleteMemberWithRelations(Long memberId) {
        queryFactory.delete(follow).where(follow.toMember.id.eq(memberId).or(follow.fromMember.id.eq(memberId))).execute();
        queryFactory.delete(notification).where(notification.receiverId.eq(memberId)).execute();
